...
DATADIR="/home/djm/PirateOcean/.zcash"
rm -rf "$DATADIR"
...
"/home/djm/PirateOcean/src/zcashd" -connect=0.0.0.0 -datadir="$DATADIR" -rpcuser=user -rpcpassword=pass -listen -keypool=3 -debug -debug=net -logtimestamps -checkmempool=0 -relaypriority=0 -port=$PORT -whitelist=127.0.0.1 -regtest -rpcport=`expr $PORT + 1` &
BITCOIND=$!
